...
### 예정 (v0.5.0 Phase 2)
- 웹 UI 에 `/projects` (목록/등록), `/projects/{id}/console` (Claude 프롬프트 + WS 로그), `/projects/{id}/builds` (빌드 트리거/목록), `/projects/{id}/files` (업로드/다운로드) 화면 추가. 안드로이드 앱이 가지고 있는 기능을 SSR 만으로 동일하게 제공.
